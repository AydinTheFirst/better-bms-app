...
const LineChart = ({ duration, frameRate, showGridLines, showXAxisLabels }: LineChartProps) => {
  ...
  const handleLongPress = useCallback(() => {
    UILog.log(`Uzun basarak grafikleri sıfırlayın`, {
      isPaused,
      chartRef,
    });
    reset();
  ...
  const handleChartClick = useCallback(
    (ev: any) => {
      ...
      UILog.log(
        isPaused ? `Grafik güncellemeleri devam ediyor` : `Grafik güncellemeleri durduldu`,
        {
          ev,
          isPaused,
          chartRef,
      ...
      if (isPaused) {
        if (status === 'connected') {
          ...
          UILog.log(`Cihaza bağlı değilken bu işlem gerçekleştirelemez`, { status, ev, isPaused });
        }
      } else {
        stop();
      }
