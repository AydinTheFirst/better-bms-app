...
const DataLoggerProvider = ({
  ...
  const stop = useCallback(() => {
    setPauseTimestamp((current) => {
      if (current !== null) {
        GlobalLog.warn(`Veri loglayacı zaten durdu.`, { current });
        return current;
      } else {
        const now = dayjs().valueOf();
        GlobalLog.info(`Veri loglayıcı durduruldu`, { now });
        return now;
      }
    });
  }, [setPauseTimestamp]);
  ...
  const start = useCallback(() => {
    ...
    setPauseTimestamp((current) => {
      ...
      if (current === null) {
        GlobalLog.warn(`Veri loglayıcı zaten açık.`, { current });
        return current;
      } else {
        return null;
      }
    ...
    if (lastPauseTimestamp) {
      ...
      GlobalLog.debug(
        `Veri Günlüğü zaman damgalarını duraklatma süresine göre düzeltme ${timeDifference}ms`,
        {
          timeDifference,
          lastPauseTimestamp,
        }
    ...
    if (liveData && pauseTimestamp === null) {
      ...
      setLiveDataLog((current) => {
        if (current.length > LOG_LIMIT) {
          GlobalLog.warn(`Performans için veri günlüğü sınırlandırıldı.`);
