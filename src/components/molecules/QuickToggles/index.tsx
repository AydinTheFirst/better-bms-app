...
const QuickToggles = ({ settingsData }: QuickTogglesProps) => {
  ...
  const handleChargeToggle = useCallback(
    async (event: ToggleEvent) => {
      if (device) {
        ...
        UILog.warn(`Cihaz şarj için hazır değil`);
      }
    },
    [device]
  );
  ...
  const handleDischargeToggle = useCallback(
    async (event: ToggleEvent) => {
      if (device) {
        ...
        UILog.warn(`Cihaz deşarj için hazır değil`);
      }
    },
    [device]
  );
  ...
  const blockInputIfLoading = useCallback<MouseEventHandler>((ev) => {
    ...
    if (isLoading.current) {
      ev.preventDefault();
      UILog.warn(`Yüklenirken işlem yapılamaz`);
    }
  }, []);
  ...
  return (
    <QuickTogglesContainer>
      ...
      <ToggleWithLabel>
        <ToggleLabel>{'Deşarj'}</ToggleLabel>
        <Toggle
          checked={discharge}
